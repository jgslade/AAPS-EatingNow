...
class GlucoseStatusProvider @Inject constructor(
    ...
    fun getGlucoseStatusData(allowOldData: Boolean = false): GlucoseStatus? {
        ...
        val bandWidth = 2                   // max allowed width; how about mmol/L conversions?
        var minBG = now.value
        var maxBG = minBG
        var cgmFlatMinutes = 0.0
        var oldDate = now.timestamp
        for (i in 1 until sizeRecords) {
            val then = data[i]
            val thenDate = then.timestamp
            minBG = Math.min(minBG, then.value)
            maxBG = Math.max(maxBG, then.value)
